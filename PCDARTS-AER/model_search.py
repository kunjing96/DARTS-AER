...
class Network(nn.Module):
  ...
  def _loss(self, input, target, entropy_reg=None):
    ...
    if entropy_reg is not None:
        weights_normal = F.softmax(self.alphas_normal[:,1:], dim=-1)
        weights_reduce = F.softmax(self.alphas_reduce[:,1:], dim=-1)
        probs = [alpha for alpha in weights_normal] + [alpha for alpha in weights_reduce]
        entropys = [entropy(p) for p in probs]
        mean_entropy = sum(entropys) / len(entropys)
        return self._criterion(logits, target) + entropy_reg * mean_entropy
    else:
