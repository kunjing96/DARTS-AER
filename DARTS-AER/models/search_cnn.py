...
class SearchCNNController(nn.Module):
    ...
    def __init__(self, C_in, C, n_classes, n_layers, criterion, n_nodes=4, stem_multiplier=3,
                 device_ids=None, init_alpha=None):
        super().__init__()
        ...
        for i in range(n_nodes):
            if init_alpha is None:
                ...
                self.alpha_normal.append(nn.Parameter(torch.Tensor([init_alpha]*(i+2))))
                self.alpha_reduce.append(nn.Parameter(torch.Tensor([init_alpha]*(i+2))))
    ...
    def forward(self, x, uniform_forward=False, alpha_epsilon=None, feature_epsilon=None):
        ...
        if alpha_epsilon is not None and alpha_epsilon != 0:
            for i, w in enumerate(weights_normal):
                w = w.add(torch.zeros_like(w).uniform_(-alpha_epsilon, alpha_epsilon))
                max_index, min = w.argmax(), w.min()
                if min.item()<0: w = w.add(-min)
                if w.sum() == 0.0: w[max_index] = 1.0
                w = w.div(w.sum())
            for i, w in enumerate(weights_reduce):
                w = w.add(torch.zeros_like(w).uniform_(-alpha_epsilon, alpha_epsilon))
                max_index, min = w.argmax(), w.min()
                if min.item()<0: w = w.add(-min)
                if w.sum() == 0.0: w[max_index] = 1.0
    ...
    def loss(self, X, y, uniform_forward=False, alpha_epsilon=None, feature_epsilon=None, entropy_reg=None):
        ...
        logits = self.forward(X, uniform_forward, alpha_epsilon, feature_epsilon)
        if entropy_reg is not None:
            weights_normal = [F.softmax(alpha[:,:-1], dim=-1) for alpha in self.alpha_normal]
            weights_reduce = [F.softmax(alpha[:,:-1], dim=-1) for alpha in self.alpha_reduce]
            probs = [alpha for alphas in weights_normal for alpha in alphas] + [alpha for alphas in weights_reduce for alpha in alphas]
            entropys = [entropy(p) for p in probs]
            mean_entropy = sum(entropys) / len(entropys)
            return self.criterion(logits, y) + entropy_reg * mean_entropy
        else:
    ...
    def op_weights(self, op_name):
        for n, p in self.net.named_parameters():
            if op_name in gt.PRIMITIVES:
                if '_ops.{:}'.format(gt.PRIMITIVES.index(op_name)) in n:
                    yield p
            else:
                assert op_name == 'others'
                if '_ops' not in n:
                    yield p

    def op_named_weights(self, op_name):
        for n, p in self.net.named_parameters():
            if op_name in gt.PRIMITIVES:
                if '_ops.{:}'.format(gt.PRIMITIVES.index(op_name)) in n:
                    yield n, p
