...
parser.add_argument('--dropout_rate', action='append', default=['0.0', '0.3', '0.6'], help='dropout rate of skip connect')
parser.add_argument('--add_width', action='append', default=['0', '12', '24'], help='add channels')
parser.add_argument('--add_layers', action='append', default=['0', '3', '6'], help='add layers')
...
def main():
    ...
    train_data = ImageNet16(args.tmp_data_dir, True , train_transform, CIFAR_CLASSES)
    ...
    for sp in range(len(num_to_keep)):
        ...
        for epoch in range(epochs):
            ...
            entropy_reg = 0.2 + (-0.2 - 0.2) * (1 + math.cos(math.pi * (epoch-eps_no_arch) / (epochs-eps_no_arch))) / 2 if epoch >= eps_no_arch else 0
            logging.info('Epoch: %d lr: %e entropy_reg: %e', epoch, lr, entropy_reg)
            ...
            if epoch < eps_no_arch:
                ...
                model.module.update_p()                
                train_acc, train_obj = train(train_queue, valid_queue, model, network_params, criterion, optimizer, optimizer_a, lr, train_arch=True, entropy_reg=entropy_reg)
...
def train(train_queue, valid_queue, model, network_params, criterion, optimizer, optimizer_a, lr, train_arch=True, entropy_reg=None):
    ...
    for step, (input, target) in enumerate(train_queue):
        ...
        if train_arch:
            ...
            if entropy_reg is not None:
                arch_param = model.module.arch_parameters()
                normal_prob = F.softmax(arch_param[0][:,1:], dim=-1)
                reduce_prob = F.softmax(arch_param[1][:,1:], dim=-1)
                probs = [prob for prob in normal_prob] + [prob for prob in reduce_prob]
                entropys = [entropy(p) for p in probs]
                mean_entropy = sum(entropys) / len(entropys)
                loss_a = criterion(logits, target_search) + entropy_reg * mean_entropy
            else:
