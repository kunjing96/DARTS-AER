...
########################################################
import os, sys, time, glob, random, argparse, math
import numpy as np
...
def backward_step_unrolled(
    ...
    if entropy_reg is not None:
        arch_param = unrolled_model.module.get_alphas()
        probs = [p for p in F.softmax(arch_param[0], dim=-1)]
        entropys = [entropy(p) for p in probs]
        mean_entropy = sum(entropys) / len(entropys)
        unrolled_loss = criterion(unrolled_logits, arch_targets) + entropy_reg * mean_entropy
    else:
...
def main(xargs):
    ...
    for epoch in range(start_epoch, total_epoch):
        ...
        logger.log(
            "\n[Search the {:}-th epoch] {:}, LR={:}, entropy_reg={:}".format(
                epoch_str, need_time, min_LR, entropy_reg
            )
        )
