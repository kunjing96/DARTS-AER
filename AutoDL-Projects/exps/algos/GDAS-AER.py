...
###########################################################################
import sys, time, random, argparse, math
...
def search_func(
    ...
    for step, (base_inputs, base_targets, arch_inputs, arch_targets) in enumerate(
        ...
        if entropy_reg is not None:
            arch_param = network.module.get_alphas()
            probs = [p for p in F.softmax(arch_param[0], dim=-1)]
            entropys = [entropy(p) for p in probs]
            mean_entropy = sum(entropys) / len(entropys)
            arch_loss = criterion(logits, arch_targets) + entropy_reg * mean_entropy
        else:
...
def main(xargs):
    ...
    for epoch in range(start_epoch, total_epoch):
        ...
        entropy_reg = 0.2 + (-0.2 - 0.2) * (1 + math.cos(math.pi * epoch / total_epoch)) / 2
        ...
        logger.log(
            "\n[Search the {:}-th epoch] {:}, tau={:}, LR={:}, entropy_reg={:}".format(
                epoch_str, need_time, search_model.get_tau(), min(w_scheduler.get_lr()), entropy_reg
            )
        )
